defmodule DenDashWeb.OrderController do
  ...
  def order_form(conn, _params) do
    ...
    is_freebie = Orders.next_order_will_be_freebie?(conn.assigns.me)

    render(conn, "order_form.html", title: "Submit Order 📝", changeset: changeset, is_freebie: is_freebie)
  end
  ...
  def create(conn, %{"order" => order}) do
    ...
    if Orders.user_has_unpaid_order(conn.assigns.me) do
      ...
      |> render("order_form.html", title: "Submit Order 📝", changeset: Order.changeset(%Order{}, order), is_freebie: is_freebie)
    else
  ...
  def pay(conn, %{"id" => id}) do
    ...
    if order.paid do
      ...
      amount = order.price
      venmo_url = "https://venmo.com/?txn=pay&audience=friends&recipients=#{recipient}&amount=#{amount}&note=DenDash:%20!#{order.venmo_note_tag}!%20Paid"
